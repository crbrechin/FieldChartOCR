...
class TopRightCorner(DETECTION):
    def __init__(self, db_config, split):
        super(TopRightCorner, self).__init__(db_config)
        ...
        self._coco_dir = os.path.join(data_dir, "top_right")

        self._label_dir = os.path.join(self._coco_dir, "annotations")
        self._label_file = os.path.join(self._label_dir, "instancesTopRight_{}.json")
        ...
        self._cache_file = os.path.join(cache_dir, "top_right_{}.pkl".format(self._dataset))
        self._load_data()
        ...
        for ind, (coco_image_id, image_id) in enumerate(tqdm(zip(coco_image_ids, self._image_ids))):
            ...
            for cat_id in self._cat_ids:
                ...
                for annotation in annotations:
                    # Convert point annotation [x, y, 6, 6] to corner format [x, y, x+6, y+6]
                    bbox = np.array(annotation["bbox"])
                    # COCO format is [x, y, width, height], convert to [x1, y1, x2, y2]
                    bbox[2] += bbox[0]  # x2 = x1 + width
                    bbox[3] += bbox[1]  # y2 = y1 + height
                    bboxes.append(bbox)
